...
(function (po) {
    ...
    po.map = function () {
        ...
        function recenter() {
            ...
            lat0 = centerRange[0].lat - y * k / tileSize.y;
            lat1 = centerRange[1].lat + y * k / tileSize.y;
            ...
            */
            center.lat = Math.max(k * centerRange[0].lat, Math.min(k * centerRange[1].lat, center.lat));
            center.lon = Math.max(k * centerRange[0].lon, Math.min(k * centerRange[1].lon, center.lon));
        }
        ...
        map.resize = function () {
            ...
            diff = {x: sizeRadius.x - o.x, y: sizeRadius.y - o.y};
            if (window.translateChildren) {
                window.translateChildren(diff.x, diff.y);
            }
            recenter();
        ...
        map.panBy = function (x, mfs, pos) {
            ...
            var newCenter = map.center({
                ...
                lat: center.lat + (angleSini * dx + angleCosi * dy) / tileSize.y
            });

            lastPos = pos || lastPos;
            if (window.translateChildren && lastPos) {
                var mouse = map.locationPoint(lastPos);
                var lc = map.locationPoint(map.center());
                translateChildren(x.x, x.y, mfs, {dx: mouse.x - lc.x, dy: mouse.y - lc.x});
            }
        ...
        map.zoom = function (x, mfs) {
            if (!arguments.length) return zoom + zoomFraction;

            if (zoom !== undefined && window.zoomChildren) {
                var l = map.locationPoint(map.center());
                zoomChildren(x - (zoom + zoomFraction), l.x, l.y, mfs);
            }
        ...
        map.zoomBy = function (z, x0, l, mfs) {
            if (arguments.length < 2) return map.zoom(zoom + zoomFraction + z, mfs);

            // compute the location of x0
            if (arguments.length < 3 || (mfs && l === undefined)) l = map.pointLocation(x0);

            // update the zoom level
            var newZoom = zoom + zoomFraction + z;

            if (window.zoomChildren) {
                var lc = map.locationPoint(map.center());
                zoomChildren(z, lc.x, lc.y, mfs);
            }
            ...
            return map.panBy({x: x0.x - x1.x, y: x0.y - x1.y}, mfs, x1);
        };
...
// See http://wiki.openstreetmap.org/wiki/Mercator
    ...
    function y2lat(y) {
        return y;//360 / Math.PI * Math.atan(Math.exp(y * Math.PI / 180)) - 90;
    }

    function lat2y(lat) {
        return lat;//180 / Math.PI * Math.log(Math.tan(Math.PI / 4 + lat * Math.PI / 360));
    }
    ...
    po.layer = function (load, unload) {
        ...
        function move() {
            ...
            if (visible && tileLevel > -5 && tileLevel < 3) {
                ...
                if (tile) {
                    scanTriangle(c0, c1, c2, scanLine);
                    scanTriangle(c2, c3, c0, scanLine);
                } else {
        ...
        layer.zoom = function (x) {
            ...
            zoom = typeof x == "function" || x == null ? x : function () {
                return x;
            };
            if (map) move();
            return layer;
...
// scan-line conversion
    function scanSpans(e0, e1, scanLine) {
...
// scan-line conversion
    function scanTriangle(a, b, c, scanLine) {
        var ab = edge(a, b),
            bc = edge(b, c),
            ca = edge(c, a);
        ...
        // scan span! scan span!
        if (ab.dy) scanSpans(ca, ab, scanLine);
        if (bc.dy) scanSpans(ca, bc, scanLine);
    }
    ...
    po.drag = function () {
        ...
        function mousemove(e) {
            if (!dragging) return;
            map.panBy({x: e.clientX - dragging.x, y: e.clientY - dragging.y}, false, {x: e.clientX, y: e.clientY});
    ...
    po.arrow = function () {
        ...
        arrow.map = function (x) {
            ...
            if (map = x) {
                parent = document;//map.focusableParent();
    ...
    po.hash = function () {
        ...
        var parser = function (map, s) {
            ...
            if (args.length < 3 || args.some(isNaN)) return true; // replace bogus hash
                ...
                map.zoomBy(args[0] - map.zoom(),
                    ...
                    {lat: /*Math.min(lat, Math.max(-lat, */args[1]/*))*/, lon: args[2]}, window.mfs && window.mfs.flag);
            }
        };
        ...
        var formatter = function (map) {
            var center = map.center(),
                zoom = map.zoom(),
                precision = Math.max(0, 2 * Math.ceil(Math.log(zoom) / Math.LN2)) || 0;
        ...
        function hashchange() {
            if (location.hash === s0 && (!window.mfs || !window.mfs.flag)) return; // ignore spurious hashchange events
